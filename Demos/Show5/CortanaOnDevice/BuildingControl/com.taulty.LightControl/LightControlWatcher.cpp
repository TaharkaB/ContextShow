...
LightControlWatcher::LightControlWatcher(AllJoynBusAttachment^ busAttachment) :
...
void LightControlWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.taulty.LightControl" };
...
void LightControlWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.taulty.LightControl"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.taulty.LightControl"),
            port);
        Added(this, args);
    }
...
void LightControlWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<LightControlWatcher>
        };
...
void LightControlWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_LightControlIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &LightControlWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void LightControlWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void LightControlWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
