...
std::map<alljoyn_interfacedescription, WeakReference*> LightControlConsumer::SourceInterfaces;

LightControlConsumer::LightControlConsumer(AllJoynBusAttachment^ busAttachment)
...
void LightControlConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void LightControlConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void LightControlConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus LightControlConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<LightControlJoinSessionResult^>^ LightControlConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ LightControlWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> LightControlJoinSessionResult^
    {
        auto result = ref new LightControlJoinSessionResult();
        result->Consumer = ref new LightControlConsumer(watcher->BusAttachment);
        result->Status = result->Consumer->JoinSession(serviceInfo);
        return result;
    });
}

IAsyncOperation<LightControlGetBuildingDefinitionJsonResult^>^ LightControlConsumer::GetBuildingDefinitionJsonAsync()
{
    return create_async([this]() -> LightControlGetBuildingDefinitionJsonResult^
    {
...
IAsyncOperation<LightControlSwitchBuildingResult^>^ LightControlConsumer::SwitchBuildingAsync(_In_ bool interfaceMemberOnOff)
{
    return create_async([this, interfaceMemberOnOff]() -> LightControlSwitchBuildingResult^
    {
...
IAsyncOperation<LightControlSwitchRoomResult^>^ LightControlConsumer::SwitchRoomAsync(_In_ Platform::String^ interfaceMemberRoomName, _In_ bool interfaceMemberOnOff)
{
    return create_async([this, interfaceMemberRoomName, interfaceMemberOnOff]() -> LightControlSwitchRoomResult^
    {
        ...
        if (ER_OK == status)
        {
            status = static_cast<QStatus>(TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(inputs, 0), "s", interfaceMemberRoomName));
        }
	
        status = static_cast<QStatus>(TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(inputs, 1), "b", interfaceMemberOnOff));
...
IAsyncOperation<LightControlToggleRoomLightResult^>^ LightControlConsumer::ToggleRoomLightAsync(_In_ Platform::String^ interfaceMemberRoomName, _In_ int32 interfaceMemberLightIndex)
{
    return create_async([this, interfaceMemberRoomName, interfaceMemberLightIndex]() -> LightControlToggleRoomLightResult^
    {
        ...
        status = static_cast<QStatus>(TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(inputs, 1), "i", interfaceMemberLightIndex));
...
void LightControlConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
...
int32 LightControlConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<LightControlConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<LightControlConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<LightControlConsumer>
    };
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "com.taulty.LightControl");
